...
| :construction: | **Simulator demo videos** | Recorded, scripted demos with zero hardware: **(1) done — the three `demos/` scenarios are terminal-recorded via [vhs](https://github.com/charmbracelet/vhs) tapes, each with a `demos/*/record.sh` so any contributor can re-render after changes (GIF + MP4 committed, embedded in the demo READMEs).** Remaining: (2) TurtleBot4 navigation in Gazebo, (3) arm pick-and-place in MuJoCo. | ![good first issue](https://img.shields.io/badge/-good%20first%20issue-7057ff) |
